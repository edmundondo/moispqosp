...
All notable changes to the Mozambique privileged admin backend are recorded here.
...
- **The site-selector dropdown/header only showed a country once it had a live row in
  `qos_reports`** — since Mozambique had none yet, the dropdown rendered blank instead of showing
  "Mozambique (mz)". `populateSiteSelect()` now always seeds every known country from
...
- **Ported the full Drill-down Explorer from `zwispqosp` v0.5.0/v0.5.1** (this app is kept as a
  ...
  analytics (City → Area → ISP → reports, now including Mozambique's real, sourced areas from
  `moispqosd` v1.1.0), ISP licensing, Benchmark reports, Moderation and Translations.
...
### Fixed
- Same rating-form truncation fix as `moispqosd` v1.1.0 (CSS grid `0fr → 1fr` expand, no fixed
